...
def test_user_authentication_updates_last_login(app):
...
def test_project_serialization_and_technology_parsing(app):
    with app.app_context():
        project = Project(
            title="Test Project",
...
def test_project_query_helpers(app):
    with app.app_context():
        db.session.add_all(
            [
